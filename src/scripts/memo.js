...
dialogSaveMemoButton.addEventListener("click", async () => {
  ...
  memoData.forEach((memo, index) => {
    ...
    li.innerHTML = DOMPurify.sanitize(`
      <button type="button" class="dialog__memo-button ${
        index === 0 ? "on" : ""
      }" data-memo-id="${memo.id}" aria-pressed="${index === 0 ? "true" : "false"}">
...
dialogDeleteMemoButton.addEventListener("click", async () => {
